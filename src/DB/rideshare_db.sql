...
/*
  This SQL file has been generated by Better Auth
...
alter table `user` add column `createdAt` datetime default CURRENT_TIMESTAMP not null;

alter table `user` add column `updatedAt` datetime default CURRENT_TIMESTAMP not null;

alter table `user` add column `twoFactorEnabled` boolean;

create table `session` (`id` varchar(36) not null primary key, `expiresAt` datetime not null, `token` varchar(255) not null unique, `createdAt` datetime not null, `updatedAt` datetime not null, `ipAddress` text, `userAgent` text, `userId` varchar(36) not null references `user` (`id`) on delete cascade, `activeOrganizationId` text);

create table `account` (`id` varchar(36) not null primary key, `accountId` text not null, `providerId` text not null, `userId` varchar(36) not null references `user` (`id`) on delete cascade, `accessToken` text, `refreshToken` text, `idToken` text, `accessTokenExpiresAt` datetime, `refreshTokenExpiresAt` datetime, `scope` text, `password` text, `createdAt` datetime not null, `updatedAt` datetime not null);

create table `verification` (`id` varchar(36) not null primary key, `identifier` text not null, `value` text not null, `expiresAt` datetime not null, `createdAt` datetime default CURRENT_TIMESTAMP, `updatedAt` datetime default CURRENT_TIMESTAMP);

create table `organization` (`id` varchar(36) not null primary key, `name` text not null, `slug` varchar(255) not null unique, `logo` text, `createdAt` datetime not null, `metadata` text);

create table `member` (`id` varchar(36) not null primary key, `organizationId` varchar(36) not null references `organization` (`id`) on delete cascade, `userId` varchar(36) not null references `user` (`id`) on delete cascade, `role` text not null, `createdAt` datetime not null);

create table `invitation` (`id` varchar(36) not null primary key, `organizationId` varchar(36) not null references `organization` (`id`) on delete cascade, `email` text not null, `role` text, `status` text not null, `expiresAt` datetime not null, `inviterId` varchar(36) not null references `user` (`id`) on delete cascade);

create table `twoFactor` (`id` varchar(36) not null primary key, `secret` text not null, `backupCodes` text not null, `userId` varchar(36) not null references `user` (`id`) on delete cascade);
